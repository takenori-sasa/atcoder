...
def main():
    w, h = [int(_x) for _x in input().rstrip().split()]
    n = int(input().rstrip())
    straw = []
    for i in range(n):
        p, q = [int(_x) for _x in input().rstrip().split()]
        straw.append((p, q))
    a = int(input().rstrip())
    ver = [int(_x) for _x in input().rstrip().split()]
    b = int(input().rstrip())
    hor = [int(_x) for _x in input().rstrip().split()]
    # minseg = SegTree2D(piece, mode='min')
    # maxseg = SegTree2D(piece, mode='max')
    # debugger(straw)
    cnt = {}
    for (x, y) in straw:
        idx, idy = bisect_left(ver, x), bisect_left(hor, y)
        if (idx, idy) not in cnt:
            cnt[(idx, idy)] = 0
        cnt[(idx, idy)] += 1
    min_cnt = min(cnt.values())
    if len(cnt) < (a+1)*(b+1):
        min_cnt = 0
    print(min_cnt, max(cnt.values()))
