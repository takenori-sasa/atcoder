...
def main():
    n, m, d = [int(_x) for _x in input().rstrip().split()]
    ...
    ans = -1
    for i in range(n):
        idx = bisect_left(b, a[i]+d)
        # debugger(i, idx)
        if idx+1 < m and abs(a[i]-b[idx+1]) <= d:
            ans = max(ans, a[i]+b[idx+1])
        if idx < m and abs(a[i]-b[idx]) <= d:
            ans = max(ans, a[i]+b[idx])
        if idx-1 >= 0 and abs(a[i]-b[idx-1]) <= d:
            ans = max(ans, a[i]+b[idx-1])
    print(ans)
