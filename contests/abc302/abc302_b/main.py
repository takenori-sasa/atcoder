...
def main():
    ...
    s = [''.join(input().rstrip().split()) for _ in range(h)]
    for x in range(h):
        for y in range(w):
            for dx, dy in DXY8:
                if x+dx*4 < 0 or y+dy*4 < 0 or x+dx*4 >= h or y+dy*4 >= w:
                    continue
                if s[x][y] == 's' and s[x+dx][y+dy] == 'n' and s[x+dx*2][y+dy*2] == 'u' and s[x+dx*3][y+dy*3] == 'k' and s[x+dx*4][y+dy*4] == 'e':
                    for k in range(5):
                        print(x+k*dx+1, y+k*dy+1)
                    return
