...
def main():
    ...
    graph = {}
    cnt = {i: set() for i in range(0, n)}
    cnt[0] = set([i for i in range(1, n+1)])
    for i in range(q):
        que = [int(_x) for _x in input().rstrip().split()]
        if que[0] == 1:
            u, v = que[1:]
            if u not in graph:
                graph[u] = set()
            if v not in graph:
                graph[v] = set()
            cnt[len(graph[u])].remove(u)
            cnt[len(graph[v])].remove(v)
            graph[u].add(v)
            graph[v].add(u)
            cnt[len(graph[u])].add(u)
            cnt[len(graph[v])].add(v)
        else:
            v = que[1]
            if v in graph:
                for c in graph[v]:
                    cnt[len(graph[c])].remove(c)
                    graph[c].remove(v)
                    cnt[max(0, len(graph[c]))].add(c)
                cnt[len(graph[v])].remove(v)
                cnt[0].add(v)
                graph[v] = set()
        print(len(cnt[0]))
