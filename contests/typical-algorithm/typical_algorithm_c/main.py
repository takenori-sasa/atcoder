...
def main():
    n = int(input().rstrip())
    graph = Graph(n, direct=True)
    for i in range(n):
        a = [int(_x) for _x in input().rstrip().split()]
        for j in range(n):
            graph.connect(i, j, a[j])
    graph.create_adjmatrix()
    debugger(graph.adjmatrix)
    ans = INF
    for p in permutations(range(1, n)):
        cost = 0
        route = [0]+list(p)+[0]
        for i in range(len(route)-1):
            cost += graph.adjmatrix[route[i]][route[i+1]]
        ans = min(ans, cost)
    print(ans)
...
class Graph():
    ...
    def connect(self, start: int, goal: int, weight: int = 1):
        self.adjlist[start].append((goal, weight))
        if not self.is_directed:
            self.adjlist[goal].append((start, weight))
        self.edge.append([weight, (start, goal)])
