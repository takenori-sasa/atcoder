...
def main():
    ...
    c = [''.join(input().rstrip().split()) for _ in range(h)]
    n = min(h, w)
    cnt = {i: 0 for i in range(1, n+1)}
    kici = [[False]*w for _ in range(h)]
    for i in range(h):
        for j in range(w):
            if c[i][j] == '#' and not kici[i][j]:
                k = 1
                while j+k < w and c[i][j+k] == '.':
                    k += 1
                if i+k < h and j+k < w and c[i+k][j+k] == '#':
                    for l in range(k):
                        if i+l >= h or j+l >= w:
                            continue
                        kici[i+l][l+j] = True
                    ...
                    # debugger(i, j, k)
                    cnt[k//2] += 1
    print(' '.join(str(c) for c in cnt.values()))
