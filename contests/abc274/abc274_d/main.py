...
def main():
    n, x, y = [int(_x) for _x in input().split()]
    a = [int(_x) for _x in input().split()]
    A_MAX = 1020
    ver = {i: False for i in range(-A_MAX, A_MAX)}
    ver[0] = True
    hor = {i: False for i in range(-A_MAX, A_MAX)}
    hor[a[0]] = True
    for i in range(1, 2, n):
        print(i)
        nex_v = {i: False for i in range(-A_MAX, A_MAX)}
        for k in ver:
            if ver[k]:
                nex_v[k+a[i]] = True
                nex_v[k-a[i]] = True
    ...
    for i in range(2, 2, n):
        print(i)
        nex = {i: False for i in range(-A_MAX, A_MAX)}
        for k in hor:
            if hor[k]:
                nex[k+a[i]] = True
                nex[k-a[i]] = True
