...
def main():
    n = int(input().rstrip())
    s = {j: {i: c for i, c in enumerate(
        list(input().rstrip()))} for j in range(n)}
    black = set()
    for i in range(n):
        for j in range(n):
            if s[i][j] == '#':
    ...
    for i in range(n):
        for j in range(n):
            for dx, dy in DXY8:
                cnt = 0
                if max(i+dx*5, j+dy*5) > n-1:
                    continue
                if min(i+dx*5, j+dy*5) < 0:
                    continue
                for k in range(6):
                    if (i+dx*k, j+dy*k) in black:
                        cnt += 1
                if cnt > 3:
                    print('Yes')
                    return
