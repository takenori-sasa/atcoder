...
'''
    ...
    field = [list(input().rstrip().split()) for _ in range(n)]
...
def main():
    ...
    q = deque([s])
    visited[s[0]][s[1]] = 0
    # debugger(q.popleft()[0])
    while q:
        aa = q.popleft()
        x = aa[0]
        y = aa[1]
        for dx, dy in DXY4:
            if x+dx < 0 or x+dx > h-1:
                continue
            if y+dy < 0 or y+dy > w-1:
                continue
            if field[x+dx][y+dy] in obs:
                continue
            if visited[x+dx][y+dy] > -1:
                continue
            visited[x+dx][y+dy] = visited[x][y]+1
            # debugger(x+dx, y+dy)
            q.append([x+dx, y+dy, visited[x+dx][y+dy]])
    debugger(visited)
